...
df1 = readtable('datasets/data_part_1.csv', 'Delimiter', ',', 'ReadVariableNames', true);
[row col] = size(df1);
display(["The fisrt dataset has ",num2str(row), " rows and ",num2str(col)," columns"]);

df2 = readtable('datasets/data_part_2.csv', 'Delimiter', ',', 'ReadVariableNames', true);
[row col] = size(df2);
...
% We check the columns in df1 that are not in df2
% and return the missing columns
miss_col = miss_column_df2_func(df1,df2);

% We add missing_columns in dataframe df1
...
save('datasets/Mat_X.mat', 'X', 'col_x', 'Y', 'col_y');
...
function [X col_x Y col_y] = split_input_output(df1)
    ...
    Y = table2array(df1(:,[12])); % We keep the predictive variable number 12 because it has less missing value with respective to the others
    col_y = df1(:,[12]).Properties.VariableNames;
end
...
function [miss_col] = miss_column_df2_func(df1,df2)
    % The dataframe must be loaded in readtable
